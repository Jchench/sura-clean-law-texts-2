...
# Define documents
documentA <- paste(readLines("machine_readable/PL104_208.txt"), collapse = "\n")
documentB <- paste(readLines("gpt_txt/PL104_208_gpt.txt"), collapse = "\n")
...
# save
PL104_208_word_count <- 
  tibble(law = "PL104_208",
...
save(PL104_208_word_count, file = "results/PL104_208_word_count.rda")

# Preprocess the documents
preprocess <- function(doc) {
  doc <- tolower(doc) # Convert to lowercase
  doc <- gsub("[[:punct:]]", "", doc) # Remove punctuation
  doc <- gsub("\\s+", " ", doc) # Normalize spaces
  return(doc)
}
...
# save
PL104_208_accuracy <- 
  tibble(law = "PL104_208", ocr = proportionA_not_in_B, gpt = proportionB_not_in_A)

save(PL104_208_accuracy, file = "results/PL104_208.rda")
